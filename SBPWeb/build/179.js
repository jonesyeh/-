...
var SchByJobPage = /** @class */ (function () {
    ...
    SchByJobPage.prototype.openNavSchPage = function (item) {
        var _this = this;
        this.global.createLoader("連線中...");
        this.global.loading.present().then(function () {
            var page;
            if (item.freq_type == 1)
                page = "SchOneDayTimePage";
            if (item.freq_type == 4 || item.freq_type > 32)
                page = "SchDayTimePage";
            if (item.freq_type == 8)
                page = "SchWeekTimePage";
            if (item.freq_type == 16)
                page = "SchMonthTimePage";
            if (item.freq_type == 32)
                page = "SchMonthLastdayTimePage";
            _this.navCtrl.push(page, { item: item, sch_no: item.sch_no });
        });
    };
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__angular_core__ = __webpack_require__(0);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_ionic_angular__ = __webpack_require__(63);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__sch_by_job__ = __webpack_require__(1651);
